...
const Settings = () => {
  const { data: session } = useSession();
  const [nightscoutAddress, setNightscoutAddress] = useState("");
  const [nightscoutApiSecret, setNightscoutApiSecret] = useState("");
  const [loading, setLoading] = useState(false);
  ...
  useEffect(() => {
    if (session?.user?.id) {
      fetch(`/api/user-settings?userId=${session.user.id}`)
        .then((res) => res.json())
        .then((data) => {
          if (data.nightscout_address) setNightscoutAddress(data.nightscout_address);
          if (data.nightscout_api_secret) setNightscoutApiSecret(data.nightscout_api_secret);
        });
    }
  }, [session]);
    ...
    <Layout>
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="bg-white p-6 rounded shadow-md w-full max-w-md">
          ...
          <input
            type="password"
            value={nightscoutApiSecret}
            onChange={(e) => setNightscoutApiSecret(e.target.value)}
